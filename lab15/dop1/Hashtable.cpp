...
{
	double key2 = 5 * ((0.6180339887499 * key) - int((0.6180339887499 * key))); // Просто две хэш функции
	key = ((int)(p + fmod(((key2 * (sqrt(5.0) - 1)) / 2), 1))) % size;
	return key;
}
//-------------------------------
...
//-------------------------------
void* Object::deleteByKey(int key)  //удаление
{
	int i = searchInd(key);
	void* t = data[i];
	if (t != NULL)
...
//-------------------------------
bool Object::deleteByValue(void* d)   //удаление
{
