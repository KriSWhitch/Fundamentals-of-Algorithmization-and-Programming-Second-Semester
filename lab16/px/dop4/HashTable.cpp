...
void HashTable::insert(const Element &elem) {   //вставка
	int hash = -1;
	hash = computeHash(elem.year, 0);
	if (find(elem) == -1) arr[hash].push_back(elem);
}

void HashTable::erase(const Element &elem) {    //удаление
...
int HashTable::find(const Element &elem) const {  //поиск
	int hash = computeHash(elem.year, 0);
...
void HashTable::displayTable() const {  //показать таблицу 
	...
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < arr[i].size(); j++) {
			cout << arr[i][j].year << " | " << arr[i][j].productName << " | " << endl;
		}
	}
}
